...
```bash
powershell scripts/run_tests_safe.ps1             # 61 тест, ~12 с, без мережі
```
...
Причина — у розділі «Де рішення ламається». Голий `python -m pytest` теж працює,
але саме він одного разу поклав редактор.
...
**Живого демо за URL поки немає.** Планувався безкоштовний Hugging Face Space,
але HF відтоді закрив безкоштовний тір для Docker- і Gradio-Spaces — лишились
тільки static. Замість платного хостингу заради демо, яке відкриють двічі, —
[запис реального прогону](docs/demo.gif) на початку цього README і `docker run`
вище, який піднімає сервіс локально за одну команду. Головний артефакт проекту
все одно не веб-інтерфейс, а таблиці в [`output/`](output/).
